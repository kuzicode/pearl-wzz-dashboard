...
- **「累计折合利润」卡片次行换成预计每小时利润**:只显示一行 `预计 +$1.79/h`(按盈亏着色,避免窄卡片换行),鼠标悬停显示算式 `当前算力预计产值 $9.25 − 租金 $7.46 = +$1.79/h`;无在跑机器时显示"暂无在跑机器";原口径说明(产出折合 − 累计租金,以及跨期口径警告)移入主数值悬浮提示。summary 新增 `profit_usd_h`,`.tip` 悬浮样式扩展到卡片次行。
