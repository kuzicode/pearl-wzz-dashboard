...
- **「累计折合利润」卡片次行换成预计每小时利润**:显示 `预计 +$1.83/h · +$43.92/日`(按盈亏着色,小时与日收益一眼可见),鼠标悬停显示算式 `当前算力预计产值 $9.29/h − 租金 $7.46/h = +$1.83/h; 按此速率 24h 约 +$43.92`;无在跑机器时显示"暂无在跑机器";原口径说明(产出折合 − 累计租金,以及跨期口径警告)移入主数值悬浮提示。summary 新增 `profit_usd_h`,`.tip` 悬浮样式扩展到卡片次行。
