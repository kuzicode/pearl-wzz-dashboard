...
ck("miner=srbminer → srb 镜像", S.effective_image(srb)=="docker.io/kuzigmgm/pearl-miner:srb-3.6.9")
...
ck("pool_of_image 认 srb 镜像为 kryptex(sniper + dashboard)", S.pool_of_image("docker.io/kuzigmgm/pearl-miner:srb-3.6.9")=="kryptex" and D.pool_of_image("kuzigmgm/pearl-miner:srb-3.6.9")=="kryptex")
...
ck("save_miner_cfg 写顶层 miner 并返回镜像", r.get("ok") and json.loads((tmp/"configs"/"config.runpod-2.json").read_text())["miner"]=="srbminer" and "srb-3.6.9" in r["image"])
