...
ck("默认变体 = srbminer, 镜像 = POOLS.image(向后兼容)", S.active_miner(base)=="srbminer" and S.effective_image(base)==S.POOLS["kryptex"]["image"]=="docker.io/kuzigmgm/pearl-miner:srb-3.6.9-r3")
r=S.pool_requires("kryptex", dict(base, miner="krig"))
ck("krig requires 含 min_cuda 13 + 池级 reliability/grace", r.get("min_cuda")==13.0 and r.get("min_reliability")==0.98 and r.get("grace_seconds_min")==1800)
ck("不传 config 按默认变体(srbminer) → 无 min_cuda", "min_cuda" not in S.pool_requires("kryptex") and S.pool_requires("kryptex").get("grace_seconds_min")==1800)
ck("miner=krig → krig 镜像", S.effective_image(dict(base, miner="krig"))=="docker.io/kuzigmgm/pearl-miner:krig-1.5.2")
srb=dict(base, miner="srbminer")
ck("miner=srbminer → srb 镜像", S.effective_image(srb)=="docker.io/kuzigmgm/pearl-miner:srb-3.6.9-r3")
...
ck("无效 miner 回退默认", S.active_miner(dict(base, miner="nope"))=="srbminer")
...
ck("pool_of_image 认 srb 镜像为 kryptex(sniper + dashboard)", S.pool_of_image("docker.io/kuzigmgm/pearl-miner:srb-3.6.9-r3")=="kryptex" and D.pool_of_image("kuzigmgm/pearl-miner:srb-3.6.9-r3")=="kryptex")
...
ck("save_miner_cfg 写顶层 miner 并返回镜像", r.get("ok") and json.loads((tmp/"configs"/"config.runpod-2.json").read_text())["miner"]=="srbminer" and "srb-3.6.9-r3" in r["image"])
