...
ck("active_pool(None) 返回默认 pearlhash", S.active_pool(None)=="pearlhash")
ck("active_pool 空白符也返回默认 pearlhash", S.active_pool({"pool":"  "})=="pearlhash")
ck("active_pool 未配 pool 时按 image 推断 kryptex", S.active_pool({"image":"docker.io/x/pearl-miner:krig-1.5.1"})=="kryptex")
