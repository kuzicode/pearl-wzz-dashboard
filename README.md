...
看板配置页顺序:「配置总览」填钱包 → 账号页 ① API Key → ② 启用 / 自动建机 → ③ 矿池(与矿机)→ ④ 最多同时租 / 时租上限 → ⑤ GPU 档(下拉选型号,行下有**参考算力 / 建议出价 / 市场参考价**,点「采用推荐」回填)→ 保存 → 重启应用。
...
- **总览**:在跑机器 / 矿池实测算力 / 累计租金 / 累计产出 / 折合利润,可按矿池切换视图;**网络产率 · 回本线 · 在跑产值 vs 时租**一行看全局;PRL/USDT K 线。
- **机器表**(每账号一张):单价 / 时长 / 算力 / $/100TH·h / **产值 $/h** / **利润率**(距回本线的距离:绿赚 · 黄回本线附近 · 红亏),表头下一条红色**回本线**;一键关闭单台。
...
| 矿池 | 矿机镜像 | 说明 |
|---|---|---|
| **PearlHash**(默认) | WildRig Multi `kuzigmgm/pearl-miner:v13-wildrig` | 池费 1%,OpenCL,对宿主驱动容错好 |
| **Kryptex** | 默认 SRBMiner-MULTI `srb-3.6.9-r3`;可选 KRig `krig-1.5.2` | PPS+ 池费 2%。SRBMiner 旧驱动宿主可跑、dev fee 2%;KRig 0% fee 但宿主需 CUDA 13(≥580 驱动),RunPod 社区机多半起不来 |

每账号「新抢矿池」/「矿机」下拉切换,只影响之后新租的机器;在跑机器保持原池继续监控回收。PearlHash 是 PRL 的算法名,各卡参考算力两个池通用。
...
| `pool` / `miner` | `pearlhash` / (Kryptex 时 `srbminer`) | 镜像随池与矿机自动决定 |
...
- Salad:需在其后台预建 container group(镜像 `kuzigmgm/pearl-miner:v13-wildrig` 或 `srb-3.6.9-r3`,env **必须填 `PRL_ADDRESS`**)+ `SALAD_API_KEY`;台数由 replica 决定,不计入上面两项护栏。
